...
def path(tree,total_depth,current_depth,Node_value,Index):
    if(current_depth==total_depth):
        print("The terminal state is",tree.rootid)
        return 0
    current_depth=current_depth+1
    x1=tree.getLeftChild()
    y1=x1.getNodeValue()
    if(Node_value==y1):
        tree=tree.getLeftChild()
        print(tree.rootid)
        path(tree,total_depth,current_depth,Node_value,Index*2)
    else:
...
l_55.setNodeValue(1)
print("The values of the tree before execution are")
printTree(myTree)
minimax_tree(myTree,4,0,True,0)
print("The path for the output value is")
path(myTree,4,0,myTree.getNodeValue(),0)
print("The values of the tree after execution are")
printTree(myTree)
